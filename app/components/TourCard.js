    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow border border-gray-100 group">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-4 text-gray-500 text-sm mb-4">
          <span className="flex items-center gap-1"><MapPin size={14}/> {tour.location_name || 'Việt Nam'}</span>
          <span className="flex items-center gap-1"><Clock size={14}/> {tour.duration_days ? `${tour.duration_days} ngày` : '3 ngày'}</span>
        </div>
